...
local ipip_ipdb_size_lower=1048576 --1M
local ipip_ipdb_size_upper=104857600 --100M

local function ipip_ipdb_valid_size(s)
	if ipip_ipdb_size_lower < s and s < ipip_ipdb_size_upper then
		return s
	else
		return nil
	end
...
local function is_boolean(obj) return type(obj) == "boolean" end
...
function _M.new(self, name, is_data)
	...
	if is_boolean(is_data) and is_data then
		data = name
		file_size = string.len(data)
	else
